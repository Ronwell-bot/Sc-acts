            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    ...
                    echo "<div class='flight'>
                            ...
                            <a href='seat-selection.php?flightNumber={$row['FlightNumber']}&departure={$row['Departure']}&arrival={$row['Arrival']}&departureTime={$row['DepartureTime']}&arrivalTime={$row['ArrivalTime']}&price={$row['Price']}&airline={$row['Airline']}&flightImage={$row['FlightImage']}' class='buy-ticket-button'>Buy Ticket</a>
                          </div>";
                }
            } else {
